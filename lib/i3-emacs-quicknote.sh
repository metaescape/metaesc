...
<< comment
用 C-alt-2 作为快捷键(i3 中设置)，检测到当前是 chrome 浏览器的话，直接发送 Y 键复制链接（sufingkey 配合, 太复杂不使用），
之后弹出 emacs 悬浮窗口打开文件跳到文件最后一行，将内容复制到 org 文件中，换行，进入 evil-insert-status 等待输入一些描述，按 F2 则关闭 emacsclient 窗口
