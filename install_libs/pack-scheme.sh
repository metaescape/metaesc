...
function install_scheme() {
	...
	if [[ $distro == "debian" ]]; then
        ...
        then
		    ...
		    print_info "从 https://download.racket-lang.org/ 中下载脚本安装 racket, 指定安装在 ~/.local/ 中能够保证用户权限"
            VERSION=8.9 # 修改此处即可更新版本
            RACKET=racket-$VERSION-x86_64-linux-cs.sh
            if [ ! -e ./$RACKET ]; then
                print_info "考虑开启 proxy"
                wget https://download.racket-lang.org/installers/$VERSION/$RACKET -O $RACKET
            fi
		...
		print_success "racket iracket 安装完毕， 通过 drracket 启动 IDE, 通过 ~/.local/bin/racket-uninstall 卸载""
